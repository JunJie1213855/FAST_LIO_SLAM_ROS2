...
int main(int argc, char **argv)
{
    ...
    // 多线程，核心
    std::thread posegraph_slam{process_pg};   // pose graph construction,  添加因子
    std::thread lc_detection{process_lcd};    // loop closure detection, 定时回环检测  scan context, 不做优化
    std::thread icp_calculation{process_icp}; // loop constraint calculation via icp, 定时 icp 获取因子图观测
